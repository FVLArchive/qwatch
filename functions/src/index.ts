...
function allowRequest(request: Request, response: Response): Promise<AllowedRequestResult> {
    if (request.headers.fulfillmentaccesstoken)
        return UserStateProviderFactory.createUserStateProvider("")
            ...
            .then(token => (token === request.headers.fulfillmentaccesstoken ? AllowedRequestResult.Continue : AllowedRequestResult.Block));
 ...
 */
export function processDialogflowFulfillment(request: Request, response: Response, initializeFirebaseConfig: () => void): Promise<Response> {
...
export const authTest = functions.https.onRequest((req, res) => {
    ...
    admin
        ...
        .push(accessToken);
    const redirectUri = `${req.query.redirect_uri}#access_token=${accessToken}&token_type='bearer'&state=${req.query.state}`;
