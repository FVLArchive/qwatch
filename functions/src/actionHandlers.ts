...
import { CacheFactory } from './dataSources/cache/cacheFactory';
import { ICache, ICachePackage } from './dataSources/cache/ICache';
...
export class ActionHandlers {
	...
	static getCacheSource(pkg: ICachePackage): Promise<ICache> {
		return CacheFactory.getCache(pkg);
	}
	...
	static getHandlerPackage(app: DialogflowApp): Promise<IActionHandlerPackage> {
		return ActionHandlers.getUserInfo(app).then(userInfo => {
			return ActionHandlers.getSource(userInfo).then(source => {
				return ActionHandlers.getCacheSource({ app: app }).then(cache => {
